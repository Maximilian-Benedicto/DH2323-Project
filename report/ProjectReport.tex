...
\author{Maximilian Benedicto \\ ***REMOVED***}
\email{mbene@kth.se}
\affiliation{%
...
\author{Arman Montazeri \\ ***REMOVED***}
...
Once the theoretical foundation was solidified, we constructed a custom rendering framework. This system was largely inspired by the programs developed in previous coursework (specifically Labs 2 and 3). We engineered a program capable of loading complex geometries in both \texttt{.obj} and \texttt{.ply} formats, alongside the standard Cornell Box scene. A central goal was to create a modular system that allowed seamless toggling between different rendering algorithms and 3D models. This approach not only reinforced ray tracing fundamentals, such as intersection handling and texture mapping, but also provided a reliable baseline against which we could compare our results after implementing the dipole method.

We began integrating the dipole method by directly translating the core mathematical formulas into code. By implementing these equations to observe their data dependencies, we were forced to reason critically about necessary data types and structures. A significant realization during this phase was that material properties vary across the RGB spectrum, necessitating independent calculations for each color channel. By using the GLM library, we treated these spectral properties as vectors, which streamlined the mathematical expressions and allowed parallel processing of all three color channels. Through this iterative coding methodology, we developed a deep intuition for how the various components interacted.
...
Maintaining camera responsiveness is a primary challenge in real-time ray tracing; computationally heavy frames can easily cause the application to stall. To mitigate this, we implemented an atomic flag: a signal that forces all active rendering threads to abort their current workload immediately if camera movement is initiated. Instead of blocking the thread to finish a stale frame, the system discards the obsolete work and prioritizes the updated view.
...
To minimize overhead, our implementation flattens the entire tree into a one-dimensional \texttt{std::vector}. The root node is stored at index $0$, and the left and right children of any node $i$ are located at indices $2i+1$ and $2i+2$, respectively. Two fundamental data structures form the backbone of this system: \texttt{AABB} and \texttt{BVHNode}.

The Axis-Aligned Bounding Box (\texttt{AABB}) defines a rectangular volume aligned with the $x$-, $y$-, and $z$-axes that conains the underlying geometry. Defined by its minimum and maximum coordinate bounds, the structure includes methods for expanding the box to contain a new point or another \texttt{AABB}.
...
In the single-scattering simulation, the penetration distance $d$ along the refracted outgoing ray is sampled from the exponential density function $\sigma_t e^{-\sigma_t d}$. This determines the internal scatter point $s$, representing the physical location where the light ray interacts with the medium. From this scatter point, a shadow ray is traced toward the light source; the intersection of this ray with the geometric boundary defines the incident entry point $x_i$. The distances from $x_i$ to $s$ and from $s$ to $x_o$ are denoted as $s_i$ and $s_o$, respectively, and are used in the radiance evaluation step.
...
Developing this rendering framework involved notable challenges but also deepened our understanding of light transport. While implementing ray-tracing structures such as BVH, intersection logic, and multithreading was relatively straightforward, bridging Jensen et al.'s theory with \texttt{C++} code required a careful, modular approach. By isolating each equation and verifying its data dependencies, we semi-successfully translated complex optical physics into a structured software architecture, paving the way for future refinements and validation against physical measurements.

\section{Contribution \& Acknowledgements}

Both members collaborated equally on all aspects of the project, including analysis, code implementation, and report writing. We utilized pair-programming techniques to ensure that both members had a good understanding of the entire codebase and theoretical framework.
